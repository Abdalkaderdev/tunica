...
import { getSectors, localeHref } from "@/lib/content";
    ...
    <section id="sectors" className="bg-ink py-24 text-cream sm:py-32">
      <div className="wrap">
        <Reveal className="max-w-2xl">
          <div className="eyebrow text-goldlight">{dict.sectorsSection.eyebrow}</div>
          <h2 className="text-3xl leading-[1.15] text-cream sm:text-4xl lg:text-[2.75rem]">
            {dict.sectorsSection.heading}
          </h2>
          <p className="mt-5 text-cream/60">{dict.sectorsSection.intro}</p>
        </Reveal>
        ...
        <div className="mt-16 divide-y divide-cream/10 border-y border-cream/10 sm:mt-20">
          {sectors.map((sector, i) => (
            <Reveal key={sector.slug} delay={i * 0.05}>
              <Link
                href={localeHref(locale, `/sectors/${sector.slug}`)}
